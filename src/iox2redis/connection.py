...
class Iox2Connection(Connection):  # type: ignore[misc]
    ...
    def send_command(self, *args, **kwargs):  # noqa: ARG002 - redis-py compatibility
        self.connect()
    ...
    def _decode_response_value(self, response):
        ...
        if isinstance(response, list) and getattr(self.encoder, "decode_responses", False):
            return [self.encoder.decode(v) if isinstance(v, bytes) else v for v in response]
        return response
    ...
    def send_packed_command(self, command, check_health: bool = True):  # noqa: ARG002
